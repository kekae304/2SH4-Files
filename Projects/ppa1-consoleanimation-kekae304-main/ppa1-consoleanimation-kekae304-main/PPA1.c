...
int charac;
char cmd;
char displayString[DISPLAY_STRING_SIZE] = {0};
int firstIndex = 0;
...
for (int i = 0; i < sizeof(displayString); i++) {
        MacUILib_printf("%c", displayString[(firstIndex + i) % (sizeof(displayString) - 1)]);
    }
    printf("\n");

    if (direction == 0) {
        firstIndex++;
        if (firstIndex >= sizeof(displayString) - 1) {
            firstIndex = 0;
        }
    } else if (direction == 1) {
        firstIndex--;
        if (firstIndex < 0) {
            firstIndex = sizeof(displayString) - 2;
        }
    }
}
...
void Initialize(void)
{
    ...
    for (int i = 4; i < sizeof(displayString); i++)
    {
        displayString[i] = ' ';
    }
...
void RunLogic(void)
{
    ...
    {
        char footer;
        if (charac < sizeof(displayString) - 3)
        {
            displayString[charac + 2] = displayString[charac + 1];
            displayString[charac] = cmd;
        }
        charac++;
    }
    ...
    // Check if there's a valid input command collected
    
    if (cmd != '\0' && ((cmd >= 'A' && cmd <= 'Z') || (cmd >= 'a' && cmd <= 'z') || (cmd >= '0' && cmd <= '9')))
     {
        if (charac < DISPLAY_STRING_SIZE - 3)
        {
            for (int i = DISPLAY_STRING_SIZE - 3; i >= charac; i--)
            {
