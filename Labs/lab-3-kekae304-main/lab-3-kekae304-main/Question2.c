...
void delete_wordlist(char **word_list, int size)
{
    ...
    {
        free(word_list[i]); //freeing the 'i' item in the array
    }
    free(word_list); //freeing the array 
}
...
{   
    ...
    for (int i = 0; i < size - 1; i++)
    {
        ...
        {
            if (my_strcmpOrder(words[j], words[j + 1]) > 0)  
            {
...
void sort_words_Selection(char **words, int size){
    ...
    for(i = 0; i < size; i++)
    {
        ...
        for(j = i + 1; j < size; j++)
        {
            if(my_strcmpOrder(words[minIndex], words[j]) == 1) //swapped i for minindex
            {
                minIndex = j;
            }                        
        }
       
        if(minIndex != i) //swapped j for i
        {
